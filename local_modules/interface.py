...
class Interface:
    ...
    # Méthode pour complêtement reset et rafraichir la fenêtre
    def refresh_main_frame(self):
        for child in self.root.winfo_children():
            child.destroy()
        self.render_main_frame()

    # Méthode pour render la fenêtre mot de passe
    def render_password_area(self):
        ...
        password_button = ttk.Button(
            password_frame, text="Se connecter", command=attempt_connection
        )
        password_button.pack(padx=4, pady=4, side=LEFT)

        password_frame.pack(padx=8, pady=8, side=LEFT)

    # Méthode pour render la fenêtre principale
    ...
    # Méthode pour le frame où va la liste
    ...
    # Méthode pour render la liste de produit
    def render_product_list(self, list_frame):
        # Fonction pour obtenir et set l'id du produit selectionné
        ...
        # Fonction pour sélectionner un attribut et ouvrir une fenêtre pour modifier
        ...
        # On utilise ttk.Treeview pour avoir plusieurs colonnes
        columns = ("id", "name", "description", "price", "quantity", "category")
        product_list = ttk.Treeview(list_frame, columns=columns)
        ...
        # Méthode pour afficher les produit dans la table
        # skip reformat black
        # fmt: off
        ...
        # Pour associer single-clic et double-clic aux fonctions
        product_list.bind("<ButtonRelease-1>", get_selection_id)
        product_list.bind("<Double-Button-1>", select_attribute)

        scrollbar = ttk.Scrollbar(
            list_frame, orient=VERTICAL, command=product_list.yview
        )
    ...
    def render_add_button(self, main_frame):
        ...
        # call fonction avec single-clic
    ...
    # Fenêtre pour mettre à jour description
    def update_description_window(self):
        def update(event):
            new_value = description_text_field.get("1.0", "end-1c")
            # empêcher description invalide
            if len(new_value) <= 0 or len(new_value) > 255:
                return
            db(self.password).update_attribute_value(
                self.selected_id, "description", new_value
            )
            self.refresh_main_frame()
        ...
        default_text = db(self.password).get_attribute_value(
            "description", self.selected_id
        )
    ...
    # fenêtre modification prix
    def update_price_window(self):
        def update(event):
            try:
                new_value = float(price_text_field.get())
                # pour empêcher valeur négative
                if new_value < 0:
                    return
                db(self.password).update_attribute_value(
                    self.selected_id, "price", new_value
                )
                self.refresh_main_frame()
            except:
                return
        ...
        def update(event):
            try:
                ...
                if new_value < 0:
                    return
                db(self.password).update_attribute_value(
                    self.selected_id, "quantity", new_value
                )
                self.refresh_main_frame()
            except:
                return
    ...
    def update_category_window(self):
        def update(event):
            ...
            db(self.password).update_attribute_value(
                self.selected_id, "id_category", id_category
            )
            self.refresh_main_frame()
        ...
        category_combobox = ttk.Combobox(
            update_category, values=db(self.password).get_categories()
        )
